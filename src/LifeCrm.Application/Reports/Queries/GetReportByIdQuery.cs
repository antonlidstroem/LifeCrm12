...
public sealed class GetReportByIdHandler : IRequestHandler<GetReportByIdQuery, MissionReportDetailDto>
{
    ...
    public async Task<MissionReportDetailDto> Handle(GetReportByIdQuery q, CancellationToken ct)
    {
        ...
        return new MissionReportDetailDto
        {
            ...
            MaterialsDistributed = r.MaterialsDistributed,
            ReviewComment = r.ReviewComment,
            ApprovedBy = r.ApprovedBy,
            DecisionTotal = r.Decisions.Sum(d => d.Count),
            PrayerCount = r.PrayerPoints.Count,
            CreatedAt = r.CreatedAt,
            SubmittedAt = r.SubmittedAt,
            ApprovedAt = r.ApprovedAt,
            Decisions = r.Decisions.Select(d => new DecisionCountDto
            {
                Id = d.Id,
                DecisionType = d.DecisionType,
                Count = d.Count
            }).ToList().AsReadOnly(),
            PeopleGroups = r.PeopleGroups.Select(p => new PeopleGroupReachedDto
            {
                Id = p.Id,
                JpCode = p.JpCode,
                PeopleGroupName = p.PeopleGroupName,
                Country = p.Country,
                Language = p.Language,
                EstimatedReached = p.EstimatedReached,
                Notes = p.Notes
            }).ToList().AsReadOnly(),
            PrayerPoints = r.PrayerPoints.Select(p => new PrayerPointDto
            {
                Id = p.Id,
                ReportId = p.ReportId,
                Title = p.Title,
